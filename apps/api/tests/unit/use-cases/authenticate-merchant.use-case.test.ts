...
describe('AuthenticateMerchantUseCase', () => {
  ...
  it('deve autenticar também quando chamado com argumentos posicionais (slug, pin)', async () => {
    const tenant = new Tenant({
      id: 'tenant-pos',
      name: 'Barbearia Posicional',
      slug: 'barbearia-pos',
    ...
    const result = await useCase.execute('barbearia-pos', '1234');
    expect(result.authenticated).toBe(true);
    expect(result.token).toBeDefined();
  });
