...
// Normalização defensiva: preserva os dados do banco e entrega 100% o layout iFood
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div
        ...
        class="bg-white text-slate-800 w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-lg flex flex-col overflow-hidden sm:rounded-3xl shadow-2xl transition-all"
        @click.stop
      >
        <!-- Top Bar com Botão Voltar e Ações -->
        <div class="p-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white sticky top-0 z-20">
          <button
            @click="emit('close')"
          ...
          <div class="flex items-center gap-3">
            <button class="p-2 rounded-full text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer" aria-label="Compartilhar">
              <Share2 class="w-5 h-5" />
            </button>
            <button class="p-2 rounded-full text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer" aria-label="Favoritar">
              <Heart class="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div class="overflow-y-auto flex-1 p-5 space-y-6 bg-white">
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between">
              <h2 id="reviews-modal-title" class="text-2xl font-black text-slate-900 tracking-tight">
                {{ storeName || 'Adega & Distribuidora Prime' }}
              </h2>
              <span class="text-[10px] font-bold px-2 py-0.5 rounded bg-red-50 text-red-600 border border-red-100 shrink-0">
                iFood Bebidas
              </span>
            </div>
          ...
          <div class="flex border-b border-slate-200">
            ...
            <button
              @click="emit('close')"
              class="pb-3 px-4 text-sm font-bold text-slate-500 hover:text-slate-800 transition-all cursor-pointer"
            >
              Informações
            </button>
          </div>
          ...
          <div class="space-y-4 pt-2">
            ...
            >
              ...
              <div class="flex items-center justify-between">
                ...
                <!-- Like / Heart -->
                <button class="flex items-center gap-1 text-slate-400 hover:text-red-500 transition-colors cursor-pointer text-xs">
                  <Heart class="w-4 h-4" />
                  <span class="text-[11px]">{{ item.likes || 0 }}</span>
                </button>
              </div>
              ...
              <div
                ...
                class="mt-3 rounded-2xl bg-[#f7f7f7] p-3.5 border border-slate-100 space-y-1.5"
              >
