    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
      ...
      <div class="space-y-4 pt-1">
        ...
        >
          ...
          <!-- Alerta Proativo caso o expediente do especialista exceda o fechamento da loja -->
          <div
            v-if="storeCloseHour && prof.workHours?.end && prof.workHours.end > storeCloseHour"
            class="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-300 flex items-start gap-2 animate-in fade-in duration-150"
          >
            <span class="text-sm shrink-0">⚠️</span>
            <span class="leading-relaxed">
              <strong>Horário estendido:</strong> Este especialista atende até às <strong>{{ prof.workHours.end }}</strong>, mas a loja fecha às <strong>{{ storeCloseHour }}</strong>. Para disponibilizar essas vagas na vitrine de agendamento, lembre-se de estender o horário de funcionamento na aba <strong>Horários & Pausa</strong>.
            </span>
          </div>
        </div>
      </div>
